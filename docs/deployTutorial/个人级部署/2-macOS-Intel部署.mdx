...
:::info 说明
部署完成后，**您并不能直接使用Xelatex编译器，也不能编译任何中文的Latex文档。**您需要参考个人级部署的第五节：[安装TexLive宏包](5-TexLive宏包的安装.md)，配置完成后你才可以使用中文Latex，或者在你的tex文档中使用各种宏。
:::
...
现在输入你的用户名和密码（密码尽可能复杂一点，简单的密码可能会被拒绝），然后确认时候再回到登陆页面，完成登陆即可。到此为止，我们的部署操作基本结束。（注意：为了安全起见，Overleaf一旦管理员页面注册成功后，再访问launchpad就会直接跳转到登录页面）
...
- 按照本教程部署的Overleaf不支持中文，也不能使用Xelatex编译器，很多宏包也没有。**关于Latex宏包安装的问题，请前往：[个人级部署-TexLive宏包的安装](./5-TexLive宏包的安装.md)**
